...
    // NOTE: We are relying on localStorage for token for now, but the API call is now centralized.
...
    useEffect(() => {
        // Check for token from localStorage (temporary until CartContext is linked)
        const token = localStorage.getItem('token');
        if (token) {
            setIsLoggedIn(true);
            fetchOrders(); // Call without passing token, as API utility handles it
        } else {
            setIsLoggedIn(false);
            setIsLoading(false);
        }
...
        try {
            // CRITICAL FIX: Use the centralized API, which handles the base URL and Authorization header
...
            // Error handling relies on the api interceptor setup
